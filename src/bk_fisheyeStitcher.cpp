                  ...
                  cv::Mat &binary_mask, std::vector<int> &blend_post )
{
    ...
    // threshold(mask_, mask_,0,255, CV_THRESH_BINARY);
    mask_.convertTo(mask_, CV_8UC3);
    //
    imwrite("ring_mask_unwarp.jpg", ring_mask_unwarped);
    // imwrite("ring_mask_unwarp_arr.jpg", ring_mask_unwarped_arr);

#if 0
    cv::Point mid_point_1{120,888};
    cv::Point mid_point_2{1804,888};
    cv::Point top_point{966,46};
    cv::Point bot_point{966,1730};
    std::vector<Point> corners;
    corners.push_back(mid_point_1); 
    corners.push_back(mid_point_2); 
    corners.push_back(top_point); 
    corners.push_back(bot_point); 

    std::vector<UMat> images;
    UMat mask_image;
    mask_.convertTo(mask_image, CV_32F);
    images.push_back(mask_image);
    ...
    UMat seam_mask;
    // seam_mask = masks.back();
    seam_mask = masks[0];

    cout << "Size (seam_mask) = " << seam_mask.size() << ", type = " << seam_mask.type() << endl;

    Mat seam_mask_;
    seam_mask.convertTo(seam_mask_, CV_8UC1);

    cout << "Size (seam_mask_) = " << seam_mask_.size() << ", type = " << seam_mask_.type() << endl;

    Mat bin_mask;
    mask_.copyTo(bin_mask, seam_mask_);
    imwrite("bin_mask.jpg", bin_mask);
#endif
    ...
    // Create post (instead of external blend_postfind.yml)
    // std::vector<int> blend_post;
    // blend_post.reserve(H_);
    // Mat blend_post(Size(1, H_), CV_16U, Scalar(0));
    int offset = 30;
    for( ridx=0; ridx < H_; ++ridx )
    {
    ...
    // for (auto it = blend_post.begin(); it != blend_post.end(); ++it) 
    // {
    //     // cout << "[" << i-blend_post.begin() << "]" << *i << " "; 
    //     cout << *it << " "; 
    // }

    // cout << "post = " << blend_post << "\n";
            ...
            const cv::Mat &binary_mask, const std::vector<int> blend_post )
{
    ...
    fs["post_ret"] >> post; // 1772 x 1
    fs.release();
    // use std::vector::blend_post instead of Mat::post
